...
export function ProfilePosts({ userId }: ProfilePostsProps) {
  ...
  const fetchPosts = async () => {
    ...
    const { data, error } = await supabase
      .from("posts")
      .select("id, image_url, likes_count, comments_count, views_count")
      .eq("user_id", userId)
      ...
      <div className="grid grid-cols-3 gap-1 md:gap-4">
        {posts.map((post) => (
          <div key={post.id} className="aspect-square relative group cursor-pointer overflow-hidden rounded">
            ...
            {/* Always Visible Views Count */}
            <div className="absolute bottom-2 left-2 bg-black/60 text-white px-2 py-1 rounded-md flex items-center gap-1 text-xs md:text-sm">
